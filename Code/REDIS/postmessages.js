...
const redis = require("redis"); // REDIS in-memory database used as data cache
let ios = undefined;
...
app.route("/messages").get(auth, (req, res, next) => {
    ...
    if (message.isMessage(recvmessage)) {
        message.getModel().create(recvmessage).then((data) => __awaiter(void 0, void 0, void 0, function* () {
            var _a, e_1, _b, _c;
            // Invalidate redis caches
            rc.del("TAGS").then(() => { console.log("TAGS REDIS cache invalidated"); });
            try {
                // Delete all cached queries (ie. keys starting with QUERY)
                for (var _d = true, _e = __asyncValues(rc.scanIterator({ TYPE: 'string', MATCH: 'QUERY*' })), _f; _f = yield _e.next(), _a = _f.done, !_a;) {
                    _c = _f.value;
                    _d = false;
                    try {
                        const key = _c;
                        yield rc.del(key).then(() => { console.log(key + " key removed"); });
                    }
                    finally {
                        _d = true;
                    }
...
app.delete('/messages/:messageid', auth, (req, res, next) => {
    ...
    message.getModel().deleteOne({ _id: mongoose.Types.ObjectId(req.params.messageid) }).then((q) => {
        if (q.deletedCount > 0)
